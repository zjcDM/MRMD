...
//	
	...
	public double average(double[][] data, int col)
	{
		double sum = 0.0;
	...
	public double varience(double[][] data, double average1, int col1, double average2, int col2)
	{
		double sum = 0.0;
		for(int i = 0; i < insNum; ++ i)
		{
			sum += (data[i][col1] - average1) * (data[i][col2] - average2);
		}
		return sum / (insNum - 1);
	}
	...
	public double[] run()
	{
		...
		for(int i = 0; i < feaNum; ++ i)
		{
			...
			for(int j = feaNum; j < labNum + feaNum; ++ j)
			{
				...
				double average1 = average(data, i);
				double average2 = average(data, j);
				numerator = varience(data, average1, i, average2, j);
				denominator = Math.sqrt(varience(data, average1, i, average1, i) * varience(data, average2, j, average2, j));
				if(Math.abs(denominator) < (1E-10))
				{
					PearsonData[i][k] = 0.0;
					continue;
				}
				this.PearsonData[i][k] = Math.abs(numerator/denominator);
